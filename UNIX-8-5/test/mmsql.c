...
void write_to_sql(struct msgbuf *pmb)
{
	MYSQL * mysql_conn = NULL;
	mysql_conn = mysql_get_conn();
	if(mysql_conn == NULL)
	...
	char sql_cmd[1024];
	sprintf(sql_cmd, "insert into student(name, password) values\
			(\'%s\',\'%s\')", pmb->stu.name, pmb->stu.password);
